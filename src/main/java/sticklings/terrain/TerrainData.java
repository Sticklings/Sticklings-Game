...
public class TerrainData {
	 ...
	 * @throws IllegalArgumentException Thrown if the size of the terrain data array does not match the given width and height
	 */
	...
	/**
	 * Locks the terrain data for read access. This does not protect against writing, please ensure you do not modify the contents. This does not have a unique lock. Make sure you call {@link #unlockRead()} when you are done
	 * 
	...
	/**
	 * Locks the terrain data for write access. This gives exclusive access to the terrain data. Make sure you call {@link #unlockWrite()} when you are done
	 * 
	 * @return The terrain data array
	 * @see #unlockWrite()
	 */
	...
	/**
	 * Marks a section of the terrain data as having been updated. This allows quick and efficient rendering of terrain
	 * 
	...
	void markDirty(BoundingBox bounds) {
		if (dirtyRegion == null) {
			...
			dirtyRegion = new BoundingBox(Math.min(dirtyRegion.getMinX(), bounds.getMinX()), Math.min(dirtyRegion.getMinY(), bounds.getMinY()), Math.max(dirtyRegion.getMaxX(), bounds.getMaxX()), Math.max(dirtyRegion.getMaxY(), bounds.getMaxY()));
		}
	}

	/**
	 * Gets and clears the dirty region of the terrain. This allows you to get the area that needs re-rendering
	 * 
	...
	/**
	 * Checks if the type of terrain at the given coords is the given type NOTE: This is relatively expensive and should be avoided when hundreds or more checks are needed at once
	 * 
	...
	/**
	 * Gets the type of terrain at the given coords. NOTE: This is relatively expensive and should be avoided when hundreds or more checks are needed at once
	 * 
	 * @param x The x coord to check
	 * @param y The y coord to check
	 * @return The type
